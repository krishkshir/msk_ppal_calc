...
import { getCurrentUser, hasSession } from "@/lib/auth/profile";
...
export default async function LoginPage(props: PageProps<"/login">) {
  ...
  // Set by requireUser (src/lib/auth/profile.ts) when a valid, signed-in
  // session isn't on the ledger's allow-list — distinct from "signed out"
  // because requesting another magic link can't fix it. Not the only way
  // to reach this state (see signedIn below), but it does get a more
  // specific message.
  const noAccess = searchParams.error === "no_access";
  // A signed-in-but-unlisted visitor can land here without the no_access
  // redirect ever firing — a bookmark straight to /login, or access
  // revoked mid-session. getCurrentUser() is null for both "signed out"
  // and "signed in but not permitted", so hasSession() (a plain session
  // check) disambiguates: only show the "wrong account" banner when
  // there's a session AND it isn't a permitted one — otherwise a
  // perfectly legitimate allow-listed visitor who just opens /login
  // directly would be told their account "isn't set up" for the ledger.
  const currentUser = await getCurrentUser();
  const signedInButNotPermitted = currentUser == null && (await hasSession());
  ...
  return (
    <main className="mx-auto max-w-sm px-6 py-16">
      ...
      {signedInButNotPermitted ? (
        <div className="mt-6 rounded-md border border-oxide/60 bg-oxide/10 px-3 py-2 text-sm text-oxide">
          <p>
            {noAccess
              ? "That account isn't set up for this ledger. Sign out and try the address you use for it."
              : "You're signed in with an account that isn't set up for this ledger. Sign out and try the address you use for it."}
          </p>
